...
function Navbar({ toggleTheme, theme }) {
  ...
  const { signed, signOut, currentUser } = useAuth();
  ...
  useEffect(() => {
    const fetchCartCount = async () => {
      if (signed && currentUser) {
        try {
          const cartItems = await getCart(currentUser.username);
          console.log(`Itens no carrinho para ${currentUser.username}:`, cartItems);
    ...
    <nav className="bg-gray-800 dark:bg-gray-200">
      <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
        ...
        <Link to="/" className="flex items-center space-x-3" onClick={handleLinkClick}>
          <span className="self-center text-4xl font-semibold whitespace-nowrap text-white dark:text-gray-800">
            DigiFlix
          </span>
        </Link>
